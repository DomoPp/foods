...
export default {
  ...
  methods: {
    ...
    async search(e) {
      let data = [];
      await axios.get("http://127.0.0.1:3000/api/foods").then(response => {
        data = response.data;
      });
      ...
      if (e !== "*" && e !== "") {
        const x = data.filter(n => {
          ...
          return regex.test(n.f_name);
        });
        // console.log(x);
        this.$refs.userTable.foods = x;
      } else {
        await axios.get("http://127.0.0.1:3000/api/foods").then(response => {
          this.$refs.userTable.foods = response.data;
        });
      }
    }
  }
